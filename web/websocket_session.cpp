...
websocket_session::websocket_session(boost::asio::ip::tcp::socket &socket) : socket_(std::move(socket)) {
}

void websocket_session::start() {
    send_text("欢迎登录websocket系统");
    on_read_frame();
}
...
void websocket_session::send_frame(websocket_opcode opcode, const std::vector<uint8_t> &payload) {
